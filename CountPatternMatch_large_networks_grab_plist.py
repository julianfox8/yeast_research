...
def reformat_output(output, tsfiles, param_dict, pgsize):
        ...
        if stablefc_bool:
            ...
            #res["stablefc"] = {e: set([]) for e in eps}
            if len(tsfiles) > 1:
        ...
        for tsfile in tsfiles:
            ...
            for param_index, data in output.items():
                ...
                if stablefc_bool:
                    ...
                    for e, b in data["match"][tsfile].items():
                        #if b:
                         #   res["stablefc"][e].add(param_index)
                        stablefc_match[e] += b
                        if len(tsfiles) > 1 and b:
            ...
            if stablefc_bool:
                ...
                #having trouble properly appending param_index into stablefc_match
        if len(tsfiles) > 1:
            if domain_bool:
                res["domain"]["all"] = sorted([[e, len(plist), pgsize] for e, plist in all_match["domain"].items()])
            if stablefc_bool:
                res["stablefc"]["all"] = {"summary" : sorted([[e, len(plist), stablefc, pgsize] for e, plist in all_match["stablefc"].items()]), "Param_list" : sorted([[ list(plist[1])] for plist in all_match["stablefc"].items()])}
                #this grabs a list of parameter values yet without the corresponding epsilons I do not have a clear understanding of what is being saved to the dictionary res
        print(res)
        return res
