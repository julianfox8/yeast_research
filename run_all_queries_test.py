...
def XC_grabber(db):
    ...
    cursor = c.cursor()
    XC_result = [ row[0] for row in cursor.execute('select ParameterIndex,Vertex from Signatures natural join ( select MorseGraphIndex,Vertex from MorseGraphAnnotations where label="XC {x, z}" except select MorseGraphIndex,Source from MorseGraphEdges);')]
