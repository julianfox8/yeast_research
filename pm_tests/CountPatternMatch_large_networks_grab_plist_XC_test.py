...
def stableFC_check(domaingraph,patterngraph):
    '''
    Check for match in any stable partial cycle for one parameter
    :param domaingraph: DSGRN domain graph object
    :param patterngraph: DSGRN pattern graph object
    :return: True or False for the existence of a match and True or False for the existence of a partial cycle
    '''
    FC = False
    ismatch = False
    morsegraph = DSGRN.MorseGraph(domaingraph)
    for i in range(0, morsegraph.poset().size()):
        if morsegraph.annotation(i)[0] == "XC {SWI4, YHP1, NDD1, SWI5}" and len(morsegraph.poset().children(i)) == 0:
            FC = True
